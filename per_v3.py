...
def validate_skill_depth(text, skill):
    """
    Verify skill is mentioned with actual work context
    Returns: (is_valid, confidence_score, evidence_count)
    """
    text_lower = text.lower()
    skill_lower = skill.lower()
    
    # LEVEL 1: Skill must appear in multiple contexts
    context_patterns = [
        # Project context
        rf'(?:project|application|system|tool).*?{skill_lower}',
        rf'{skill_lower}.*?(?:project|built|developed|created)',
        
        # Work context  
        rf'(?:worked with|experience in|proficient in|expert in).*?{skill_lower}',
        rf'{skill_lower}.*?(?:development|implementation|deployment)',
        
        # Achievement context
        rf'(?:using|with|via).*?{skill_lower}.*?(?:improved|increased|reduced|achieved)',
        
        # Technical context
        rf'{skill_lower}.*?(?:framework|library|database|platform|tool)',
    ]
    ...
    # LEVEL 2: Check for depth indicators
    depth_indicators = [
        # Expertise levels
        rf'(?:expert|advanced|proficient|senior).*?{skill_lower}',
        rf'{skill_lower}.*?(?:expert|advanced|proficient)',
        
        # Certifications
        rf'{skill_lower}.*?(?:certified|certification)',
        
        # Version specificity (shows actual knowledge)
        rf'{skill_lower}\s*(?:\d+\.?\d*|ES6|ES2015)',
    ]
    ...
    # SCORING
    if evidence_count == 0:
        return False, 0, 0  # Just listed, no context
    elif evidence_count == 1:
        return True, 30, evidence_count  # Minimal evidence
    elif evidence_count == 2:
        return True, 60, evidence_count  # Moderate evidence
    else:
        confidence = min(100, 70 + (depth_score * 10))
        return True, confidence, evidence_count


def validate_skill_experience_ratio(skills_count, experience_years):
    """
    Check if skill count is realistic for experience level
    """
    # Expected skill ranges
    skill_ranges = {
        0: (3, 10),      # Fresher
        1: (5, 12),      # 1 year
        2: (7, 15),      # 2 years
    ...
    if experience_years <= 5:
        min_expected, max_expected = skill_ranges.get(experience_years, (5, 15))
    elif experience_years <= 10:
        min_expected, max_expected = skill_ranges[10]
    else:
        min_expected, max_expected = skill_ranges[15]
    
    # Calculate ratio score
    if skills_count < min_expected:
        ratio_score = (skills_count / min_expected) * 100
        verdict = "Below expected"
    elif skills_count <= max_expected:
        ratio_score = 100
        verdict = "Appropriate"
    else:
        # Penalty for excessive skills
        excess = skills_count - max_expected
        penalty = min(50, excess * 3)  # -3 points per excessive skill, max -50
        ratio_score = max(0, 100 - penalty)
        verdict = "Suspicious - Too many skills for experience level"
    ...
    return {
        ...
        'is_suspicious': skills_count > max_expected * 1.5
    }
...
def validate_skills_in_projects(text, skills):
    """
    Check if claimed skills are actually used in projects
    """
    # Extract project section
    project_section = extract_section(text, 
        start_markers=['project', 'experience', 'work'],
        end_markers=['education', 'skills', 'certification']
    )
    ...
    if not project_section:
        return {
            ...
            'warning': 'No project section found'
        }
    ...
    for skill in skills:
        # Check if skill appears in project context
        skill_pattern = rf'\b{re.escape(skill)}\b'
        
        # Look for skill near action verbs
        context_pattern = rf'(?:built|developed|created|implemented|designed|deployed).*?{skill_pattern}|{skill_pattern}.*?(?:built|developed|created|implemented|designed|deployed)'
        
        if re.search(context_pattern, project_section, re.IGNORECASE):
            verified_skills.append(skill)
        else:
            # Check if just mentioned in project section
            if re.search(skill_pattern, project_section, re.IGNORECASE):
                verified_skills.append(skill)
            else:
                unverified_skills.append(skill)
    
    correlation_percentage = (len(verified_skills) / len(skills) * 100) if skills else 0
    
    return {
        'correlation_score': correlation_percentage,
        'verified_skills': verified_skills,
        'unverified_skills': unverified_skills,
        'verification_rate': f"{len(verified_skills)}/{len(skills)}"
    }


def validate_skill_achievements(text, skill):
    """
    Check if skill has measurable outcomes/achievements
    """
    # Patterns for achievements with metrics
    achievement_patterns = [
        # Performance improvements
        rf'(?:using|with|via)\s+{re.escape(skill)}.*?(?:improved|increased|reduced|optimized).*?(\d+%)',
        
        # Scale metrics
        rf'{re.escape(skill)}.*?(?:handled|processed|managed).*?(\d+(?:,\d+)?)\s+(?:users|requests|records|transactions)',
        
        # Time metrics
        rf'{re.escape(skill)}.*?(?:reduced|decreased).*?(?:time|latency|load).*?(\d+%)',
        
        # Business impact
        rf'{re.escape(skill)}.*?(?:generated|saved|increased revenue).*?\$(\d+)',
    ]
...
def calculate_authenticity_score(resume_data, resume_text):
    """
    Multi-factor authentication score
    Combines all validation layers
    Returns 0-100 score (100 = most authentic)
    """
    ...
    if not skills:
        return {
            ...
            'risk_level': 'Medium',
            'breakdown': {},
            'red_flags': ['No skills detected'],
            'verified_skills': 0,
            'total_skills': 0,
            'verification_rate': '0%'
        }
    
    # FACTOR 1: Context validation (40 points max)
    context_scores = []
    verified_skills = 0
    for skill in skills[:20]:  # Check top 20 skills to avoid performance issues
        is_valid, confidence, evidence = validate_skill_depth(resume_text, skill)
        if is_valid:
            verified_skills += 1
            context_scores.append(confidence)
    
    avg_context_score = sum(context_scores) / len(context_scores) if context_scores else 0
    context_points = (avg_context_score / 100) * 40
    
    # FACTOR 2: Experience-skill ratio (25 points max)
    ratio_result = validate_skill_experience_ratio(len(skills), experience_years)
    ratio_points = (ratio_result['ratio_score'] / 100) * 25
    
    # FACTOR 3: Project correlation (20 points max)
    correlation_result = validate_skills_in_projects(resume_text, skills)
    correlation_points = (correlation_result['correlation_score'] / 100) * 20
    
    # FACTOR 4: Achievement evidence (15 points max)
    total_achievements = 0
    for skill in skills[:10]:  # Check top 10 skills
        achievement_result = validate_skill_achievements(resume_text, skill)
        total_achievements += achievement_result['achievement_count']
    
    # Award points based on achievement density
    achievement_points = min(15, total_achievements * 3)
    
    # TOTAL AUTHENTICITY SCORE
    authenticity_score = int(context_points + ratio_points + 
                            correlation_points + achievement_points)
    
    # FRAUD DETECTION FLAGS
    red_flags = []
    
    if ratio_result['is_suspicious']:
        red_flags.append("⚠️ Skill count too high for experience level")
    
    checked_skills = min(20, len(skills))
    if checked_skills > 0 and verified_skills / checked_skills < 0.5:
        red_flags.append("⚠️ Over 50% of skills lack contextual evidence")
    
    if correlation_result['correlation_score'] < 40:
        red_flags.append("⚠️ Many skills not demonstrated in projects")
    
    if total_achievements == 0 and len(skills) > 10:
        red_flags.append("⚠️ No quantifiable achievements despite many skills")
    ...
    return {
        ...
        'breakdown': {
            'context_validation': f"{context_points:.1f}/40",
            'experience_ratio': f"{ratio_points:.1f}/25",
            'project_correlation': f"{correlation_points:.1f}/20",
            'achievements': f"{achievement_points:.1f}/15"
        },
        'red_flags': red_flags,
        'verified_skills': verified_skills,
        'total_skills': len(skills),
        'verification_rate': f"{(verified_skills/min(20, len(skills))*100):.1f}%" if skills else "0%"
    }
        ...
        if pdf_file:
            ...
            with col2:
                ...
                if resume_data is None:
                    ...
                    # Calculate authenticity score (fraud detection)
                    authenticity_result = calculate_authenticity_score(
                        resume_data, resume_data['raw_text']
                    )
                    ...
                    risk_class = f"risk-{authenticity_result['risk_level'].lower().replace('-', '')}"
                    st.markdown(f"""
                        <div class='authenticity-card'>
                            <div class='authenticity-score'>{authenticity_result['authenticity_score']}</div>
                            <div class='authenticity-label'>Authenticity Score (Fraud Detection)</div>
                            <div class='authenticity-verdict {risk_class}'>{authenticity_result['verdict']}</div>
                            <div style='font-size: 0.9rem; color: #666; margin-top: 0.5rem;'>
                                Risk Level: <strong>{authenticity_result['risk_level']}</strong> | 
                                Skills Verified: <strong>{authenticity_result['verification_rate']}</strong>
                            </div>
                        </div>
                    """, unsafe_allow_html=True)
                    
                    # Add authenticity breakdown in expander
                    with st.expander("🔍 View Authenticity Details"):
                        st.markdown("**Score Breakdown:**")
                        for component, value in authenticity_result['breakdown'].items():
                            st.write(f"• {component.replace('_', ' ').title()}: {value}")
                        
                        st.markdown(f"""
                        **Verification Stats:**
                        - Total Skills: {authenticity_result['total_skills']}
                        - Verified Skills: {authenticity_result['verified_skills']}
                        - Verification Rate: {authenticity_result['verification_rate']}
                        """)
                        
                        if authenticity_result['red_flags']:
                            st.markdown("**⚠️ Red Flags Detected:**")
                            for flag in authenticity_result['red_flags']:
                                st.warning(flag)
                    ...
                    # Add authenticity feedback first if there are concerns
                    if authenticity_result['risk_level'] in ['High', 'Critical', 'Medium']:
                        st.markdown(f"<div class='feedback-box feedback-warning'>🔍 Authenticity Check: {authenticity_result['verdict']}</div>", 
                                  unsafe_allow_html=True)
                        if authenticity_result['red_flags']:
                            for flag in authenticity_result['red_flags'][:2]:  # Show first 2 flags
                                st.markdown(f"<div class='feedback-box feedback-warning'>{flag}</div>", 
                                          unsafe_allow_html=True)
                    
                    # Regular ATS feedback
